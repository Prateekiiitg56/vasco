...
- **Vite + React 19**: Clean module setup configured for high performance and fast developme# Vasco Project — Implementation & Progress Summary
...
## ✅ Verified: girl-character.glb's baked animation is the kick move
Sampled bone world-positions frame-by-frame for both `assets/girl-character.glb`'s
baked clip and the loose `Chapa_Giratoria_2.fbx` in the repo root:

- Same duration (2.37s vs 2.35s), same `mixamorig:` skeleton naming.
- Both show the trailing foot rising *above* hip height at the same relative
  timestamp (~79% through the clip) — the signature of a Chapa Giratória
  spinning kick.

**Conclusion:** the kick is already baked into the girl model's one animation
clip — no extra wiring needed. Because `GirlCharacter.jsx` maps scroll
progress linearly to clip time, that kick apex currently lands at ~79% of
total page scroll, inside **"04 — Culture"**, just before the final section.

**Cleanup:** `Chapa_Giratoria_2.fbx` (21MB) is not imported anywhere in the
app — it's leftover source material from building the girl model's
animation. Safe to delete from the repo unless you want to keep it as an
editable Blender/Mixamo source.nt reload.
- **Dependencies**:
  - `@splinetool/react-spline` & `@splinetool/runtime` for 3D interactive web graphics.
  - `framer-motion` for fluid section entrance/exit animations.
  - `Vite` & standard plugins.
